...
if __name__ == '__main__':
    for node in drm.devices():
        if isinstance(node, drm.CardDevice):
            ...
            print('capabilities:')
            print('  dumb buffers:', device.get_capability(drm.DRM_CAP_DUMB_BUFFER))
            print('  VBLANK high CRTC:', device.get_capability(drm.DRM_CAP_VBLANK_HIGH_CRTC))
            print('  preferred depth:', device.get_capability(drm.DRM_CAP_DUMB_PREFERRED_DEPTH))
            print('  prefer shadow:', device.get_capability(drm.DRM_CAP_DUMB_PREFER_SHADOW))

            prime = device.get_capability(drm.DRM_CAP_PRIME)
            flags = []

            if prime & drm.DRM_CAP_PRIME_IMPORT:
                flags.append('import')

            if prime & drm.DRM_CAP_PRIME_EXPORT:
                flags.append('export')

            print('  PRIME:', ', '.join(flags))
            print('  timestamp monotonic:', device.get_capability(drm.DRM_CAP_TIMESTAMP_MONOTONIC))
            print('  async page flip:', device.get_capability(drm.DRM_CAP_ASYNC_PAGE_FLIP))

            width = device.get_capability(drm.DRM_CAP_CURSOR_WIDTH)
            height = device.get_capability(drm.DRM_CAP_CURSOR_HEIGHT)
            print('  cursor: %ux%u' % (width, height))

            print('  framebuffer modifiers:', device.get_capability(drm.DRM_CAP_ADDFB2_MODIFIERS))
            print('  page flip target:', device.get_capability(drm.DRM_CAP_PAGE_FLIP_TARGET))
            print('  CRTC in VBLANK event:', device.get_capability(drm.DRM_CAP_CRTC_IN_VBLANK_EVENT))
            print('  Sync objects:', device.get_capability(drm.DRM_CAP_SYNCOBJ))
